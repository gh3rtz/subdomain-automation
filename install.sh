...
sudo apt install -y curl git jq python3 python3-pip unzip
...
# Function to install Go tools safely
install_go_tool() {
    local tool_name="$1"
    local tool_repo="$2"
...
# ProjectDiscovery & other recon tools
...
# Optional: Create common recon directories
mkdir -p ~/Tools ~/myWordlists/fuzz4bounty/DNS ~/myWordlists/resolvers

print_message "$GREEN" "✅ Installation complete!"
print_message "$GREEN" "All tools are now available in /usr/local/bin."
print_message "$GREEN" "Make sure to add your resolvers and wordlists before running h3rtzv13."
